package com.rahul.connect.network
...
import com.rahul.connect.core.DeviceSnapshot
...
class RahulWebSocketClient(
    ...
    private fun sendHello() {
        send(RahulProtocol.hello(buildSnapshot()))
    ...
    private fun sendPairRequest() {
        ...
        send(RahulProtocol.envelope(RahulProtocol.PAIR_REQUEST, snapshot))
    ...
    private fun sendAuthenticate() {
        ...
        currentCredential = credential
        send(RahulProtocol.authenticate(credential))
    ...
    private fun handleCommandMessage(root: JSONObject) {
        ...
        val response = RahulProtocol.envelope(
            RahulProtocol.RESULT,
            result.toJson(),
            requestId = requestId,
        )
        send(response)
    ...
    private inner class RahulSocketListener : WebSocketListener() {
        ...
        override fun onMessage(webSocket: WebSocket, text: String) {
            runCatching {
                ...
                when (type) {
                    RahulProtocol.PAIR_REQUEST -> {
                        AgentStateStore.setStatus("Waiting for approval")
                    }
                    RahulProtocol.PAIR_APPROVED -> handlePairApproved(root)
                    RahulProtocol.DEVICE_ONLINE -> {
                        AgentStateStore.setConnectionState(ConnectionState.CONNECTED)
                        AgentStateStore.setStatus("Connected")
                    }
                    RahulProtocol.CAPABILITIES -> {
                        AgentStateStore.addLog("Capabilities synced")
                    }
                    RahulProtocol.EXECUTE -> handleCommandMessage(root)
                    RahulProtocol.PING -> {
                        send(RahulProtocol.envelope(RahulProtocol.PONG, JSONObject().put("status", "ok"), requestId = root.optString("request_id")))
                    }
                    RahulProtocol.ERROR -> {
                        AgentStateStore.setError(root.optJSONObject("payload")?.optString("error"))
                    }
                    RahulProtocol.CHAT_MESSAGE -> {
    ...
    fun sendChatMessage(text: String) {
        val payload = RahulProtocol.chatMessage(text)
