"""
dashboard/server.py — Rahul Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "Rahul Uploads",
        Path.home() / "Documents" / "Rahul Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'RAHUL-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"Rahul Dashboard Port {port}"
        prog_rule  = "Rahul Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="rahul_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until Rahul is run as Administrator.")
...
def _ensure_ssl_certs() -> bool:
    ...
    key_path = certs / "rahul.key"
    cert_path = certs / "rahul.crt"
    ...
    try:
        ...
        subject = issuer = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "Rahul AI Local Remote"),
        ])
        alt_names = [
...
<p>Press <strong style="color:#dde3ed">Mobile Connect</strong> in Rahul to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('rahul_token','{tok}');
  sessionStorage.setItem('rahul_key','{key}');
  localStorage.setItem('rahul_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to Rahul…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "rahul.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "rahul.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Mobile Connect' in Rahul UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
