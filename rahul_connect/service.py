...
from .gateway.server import RahulGateway, RahulGatewayConfig


@dataclass(slots=True)
class RahulConnectService:
    """Small runtime wrapper around the gateway server."""

    base_dir: Path
    config: RahulGatewayConfig | None = None
    gateway: RahulGateway = field(init=False)
    _thread: threading.Thread | None = field(init=False, default=None)
    _started: bool = field(init=False, default=False)

    def __post_init__(self) -> None:
        self.base_dir = Path(self.base_dir)
        self.gateway = RahulGateway(self.base_dir, self.config)
    ...
    def start_background(self) -> None:
        ...
        self._thread = threading.Thread(target=_runner, name="RahulConnectGateway", daemon=True)
        self._thread.start()
        self._started = True
    ...
    async def disconnect_device(self, target: str, *, reason: str = "Disconnected by Rahul") -> dict[str, Any]:
...
_SERVICE: RahulConnectService | None = None


def get_service(base_dir: str | Path) -> RahulConnectService:
    global _SERVICE
    if _SERVICE is None:
        _SERVICE = RahulConnectService(Path(base_dir))
    return _SERVICE
